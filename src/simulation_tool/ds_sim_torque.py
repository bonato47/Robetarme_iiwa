...
VERBOSE = False
PATH_SCENE = current_dir + "/ds_trial_torque.ttt"
FREQUENCY = 100
...
def init_robot_pos(sim: any, robot: Robot):
    ...
    for i in range(robot.nb_dofs):
        ...
        sim.setJointTargetForce(robot.lst_joints[i], INIT_FORCE[i])
...
def set_torque(sim: any, robot: Robot):
    ...
    """
    for i in range(robot.nb_dofs):
        sim.setJointTargetForce(robot.lst_joints[i], cmd_torques[i])
